(function(){
    /* =====================================================================
     * order_delivery_v6 (2026-06-17) — 주문서 배송방법 "잠금"(PPT 규칙2) + 화물 안내문(PPT 슬라이드4)
     *  - 잠금: v5는 택배만 숨겼으나, v6는 상세에서 선택한 배송방법 1개만 남기고 나머지 전부 숨김 = 변경 불가.
     ...
     *  - 미경유/불명이면 손대지 않음(카페24 기본). 표시·선택만, 제출/청구 로직 불변. (ndDelivCompanyRadios/applyOrderShipping)
    ...
    /* [추가 2026-06-15] 주문서 배송업체 라디오(name="delivcompany") 전용 매핑 — 가꿈 요청 2번
     *  실제 구조(주문서 콘솔 확인):
     *    <span class="ec-base-label">
     *      <input name="delivcompany" id="delivcompanyN" value="12|17|18" type="radio">
     *      <label for="delivcompanyN">택배배송(선불) | 화물배송 (착불) | 방문수령</label>
     *    </span>
     *  → label이 input의 "형제"(label[for])라 radio.closest('label')=null → 기존 자동선택이 실패했던 원인.
     *  여기서는 label[for]로 텍스트를 읽어 방법(kind)을 정확히 판별한다. (표시/선택 판별용, value 불변) */
    function ndDelivCompanyRadios(order){
        ...
        return Array.prototype.map.call(radios, function(r){
            ...
            var txt = lbl ? trimText(lbl.textContent) : (r.value || "");
            return {
                radio: r,
    ...
    function enhanceOrderForm(){
        ...
        function render(){
            ...
            if(detected.method === "화물배송(착불)"){
                ...
                if(/착불/.test(ndFeeTxt) || /^0\s*원?$/.test(ndFeeTxt) || ndFeeTxt === ""){
                    if(ndFeeEl && ndFeeEl.textContent.indexOf("별도") < 0){
                        ndFeeEl.textContent = "착불 (배송비 별도)";
                    }
                    var ndInfoEl = order.querySelector("#deliv_company_shipping_info");
                    if(ndInfoEl && ndInfoEl.textContent.indexOf("해피콜") < 0){
                        ndInfoEl.textContent = "⚠️ 현재 결제금액에는 배송비가 포함되어 있지 않습니다. 배송비 및 배송일정은 지역, 수량, 배송 상황에 따라 달라질 수 있으며, 주문 후 해피콜을 통해 별도 안내해 드립니다.";
                    }
                }
            }
        }
        ...
        /* [추가 2026-06-15] 주문서 배송업체(delivcompany) 처리 — 가꿈 요청 2번
         *  상세에서 "명시적으로" 화물/방문을 선택(localStorage)하고 온 경우에만 작동:
         *   ① 그 방법 라디오를 click()으로 선택 → 카페24가 배송비 재계산(화물=착불, 잘못된 선불청구 차단)
         *   ② 택배배송(선불) 라디오를 display:none 으로 숨김(상세가 택배가 아니면)
         *  안전장치:
         *   - localStorage 미설정(상세 미경유)이면 즉시 종료 → 카페24 기본 그대로(손대지 않음)
         *   - 자동 click은 최대 5회(무한 클릭루프 방지). 숨김은 idempotent라 매번 재적용해도 무해
         *   - 라디오 '선택'과 '숨김'만 — input value/name/제출은 카페24 그대로 */
        var ndAutoClicks = 0;
        function applyOrderShipping(){
            var items = ndDelivCompanyRadios(order);
            if(!items.length) return false;
            var rawStored = normalizeMethod(storageGet("ndDeliveryMethod", ""));
            if(!rawStored) return false;                  // 상세 미경유/불명 → 손대지 않음(안전)
            var storedKind = methodKind(rawStored);        // parcel / freight / pickup
            var target = null;
            for(var i = 0; i < items.length; i += 1){
                if(items[i].kind === storedKind){ target = items[i]; break; }
            }
            // ① 상세 선택값 라디오 선택(아직이면) — click()으로 카페24 배송비 재계산 트리거
            ...
            //    (택배 미선택인데 화물-only 제품 등으로 target 불일치면 전부 노출 유지 → 빈 선택 방지)
            if(target){
                for(var j = 0; j < items.length; j += 1){
                    if(items[j].wrap && items[j].wrap.style){
                        items[j].wrap.style.display = (items[j].kind === storedKind) ? "" : "none";
                    }
                }
            }
            return !!(target && target.radio.checked);
        }

        function sync(){
            var applied = applyStoredShipping();
            var ndApplied = applyOrderShipping();
            render();
            return applied || ndApplied;
        }
