...
(function () {
  ...
  var BACKEND = "https://warranty.gaggum.kr";
  ...
  function productNoFromUrl(href) {
    if (!href) return null;
    try {
      var value = new URL(href, window.location.href).searchParams.get("product_no");
  ...
  function currentProductItem() {
    ...
    var image = document.querySelector(
      ".xans-product-detail .keyImg img, .detailArea .keyImg img, .imgArea .keyImg img",
    );
  ...
  function additionalProductImage(productNo, productName) {
    ...
    var card = select ? select.closest(".productSet") : null;
    var cardName = cleanText(card && card.querySelector("strong.name")
      ? card.querySelector("strong.name").textContent
      : "");
  ...
  ready(function () {
    ...
    function openModal() {
      ...
      var detailItems = cartItems.length ? [] : readDetailItems();
      var automaticItems = cartItems.length ? cartItems : detailItems;
