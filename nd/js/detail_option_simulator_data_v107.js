(function(){
    ...
    // Skin16 uses the public warranty API as the source of truth.
    // Legacy FTP CSV/static product-name fallback is off unless explicitly enabled.
    window.ND_OPTION_SIMULATOR_API_BASE = window.ND_OPTION_SIMULATOR_API_BASE || "https://warranty.gaggum.kr";
    window.ND_OPTION_SIMULATOR_ALLOW_LEGACY_FALLBACK = window.ND_OPTION_SIMULATOR_ALLOW_LEGACY_FALLBACK === true;
        ...
        productNameRules: [
            {
                ...
                config: {
                    productFamily: "ladder-allo",
                    imageBasePath: "/nd/images/simulator/products/{productNo}",
                    showOnLoad: false,
                    groups: [
                        {
                            key: "topSize",
                    ...
                    layers: [
                        {
                            key: "frame",
                            order: 0,
                            imageTemplate: "/nd/images/simulator/products/{productNo}/frame/{topSize}-{frameColor}.png",
                            valueMap: {
                                topSize: {
                                    "basic": "basic-plus",
                                    "plus": "basic-plus",
                        ...
                        },
                        {
                            key: "top",
                            order: 1,
                            imageTemplate: "/nd/images/simulator/products/{productNo}/top/{topSize}-{topShape}-{topColor}.png"
                        },
                        {
                            key: "seat",
                            order: 2,
                            imageTemplate: "/nd/images/simulator/products/{productNo}/seat/{seatColor}.png"
                        }
                    ]
                }
            }
    ...
    (function addSlashAlloFallback(){
        var data = window.ND_OPTION_SIMULATOR;
        var rules = data && data.productNameRules ? data.productNameRules : [];
        var baseRule = rules[0];
        if(!baseRule || !baseRule.config) return;

        function cloneSlashConfig(family){
            var config = JSON.parse(JSON.stringify(baseRule.config));
            config.productFamily = family;
            config.imageBasePath = "/nd/images/simulator/products/1034";

            (config.layers || []).forEach(function(layer){
                if(layer.imageTemplate){
                    layer.imageTemplate = layer.imageTemplate.replace("/nd/images/simulator/products/{productNo}", "/nd/images/simulator/products/1034");
                }
            });
            return config;
        }
        ...
        rules.unshift({
            ...
            excludeKeywords: ["납품사례"],
            config: cloneSlashConfig("slash-allo")
        });
        ...
        rules.unshift({
            ...
            excludeKeywords: ["납품사례"],
            config: cloneSlashConfig("slash-woo")
        });
    }());
