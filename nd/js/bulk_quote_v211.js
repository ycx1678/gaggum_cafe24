...
(function () {
  ...
  function makeItemRow(name, qty, meta) {
    ...
    if (meta.variantImageLayersJson) {
      row.setAttribute("data-variant-image-layers-json", meta.variantImageLayersJson);
    }
  ...
  function matchToken(text) {
    return cleanText(text).toLowerCase().replace(/[^a-z0-9가-힣]/g, "");
  }

  function simulatorSnapshots() {
    try {
      var saved = JSON.parse(
        window.localStorage.getItem("gaggum_quote_simulator_snapshots") || "[]",
      );
  ...
  function simulatorSnapshotForItem(productNo, productOptions) {
    if (!(productNo > 0)) return null;
    var target = matchToken(productOptions);
    var candidates = simulatorSnapshots().filter(function (snapshot) {
      return snapshot && String(snapshot.productNo) === String(productNo) &&
        Array.isArray(snapshot.layers) && snapshot.layers.length > 0;
    });
    if (!candidates.length) return null;

    var matched = candidates.filter(function (snapshot) {
      var values = Array.isArray(snapshot.selectedValues) ? snapshot.selectedValues : [];
      var tokens = values.map(matchToken).filter(Boolean);
      return target && tokens.length > 0 && tokens.every(function (token) {
    ...
    matched.sort(function (a, b) {
      return (parseInt(b.capturedAt, 10) || 0) - (parseInt(a.capturedAt, 10) || 0);
    });
    return matched[0];
  }
  ...
  function readCartItems() {
    ...
    Array.prototype.forEach.call(rows, function (row) {
      ...
      var productNo = productNoFromUrl(link ? link.getAttribute("href") : "");
      var productOptions = optionTexts.length ? optionTexts.join(" / ") : null;
      var simulatorSnapshot = simulatorSnapshotForItem(productNo, productOptions);
      items.push({
        productName: name,
        ...
        imageUrl: absoluteImage(image ? image.getAttribute("src") : ""),
        variantImageLayersJson: simulatorSnapshot
          ? JSON.stringify({ layers: simulatorSnapshot.layers })
          : null,
  ...
  ready(function () {
    ...
    var cachedVerifiedMemberProfile = null;
    var verifiedMemberProfileRequest = null;
    ...
    function applyMemberProfile(profile) {
      ...
      if (form.companyName && !form.companyName.value.trim() && profile.companyName) {
    ...
    function loadVerifiedMemberProfile() {
      if (!memberIdInput.value) return Promise.resolve(null);
      if (cachedVerifiedMemberProfile) {
        applyMemberProfile(cachedVerifiedMemberProfile);
        return Promise.resolve(cachedVerifiedMemberProfile);
      }
      if (verifiedMemberProfileRequest) {
      ...
      var proofItems = readCartItems()
        .filter(function (item) {
          return item.productNo > 0 && item.qty > 0;
        })
        .map(function (item) {
          return { productNo: item.productNo, qty: item.qty };
        });
          ...
          return response
            ...
            .then(function (body) {
              if (!response.ok || !body || !body.ok || !body.data) {
                throw new Error("MEMBER_PROFILE_NOT_VERIFIED");
              }
              return body.data.profile || null;
            });
        })
        .then(function (profile) {
          cachedVerifiedMemberProfile = profile;
          verifiedMemberProfileRequest = null;
          return profile;
        })
        .catch(function () {
          verifiedMemberProfileRequest = null;
    ...
    form.addEventListener("submit", function (e) {
        ...
        function (row) {
          ...
          if (nm) {
            ...
            items.push({
              ...
              variantImageLayersJson:
                row.getAttribute("data-variant-image-layers-json") || null,
      ...
      var payload = {
        ...
        preferredDeliveryDate: form.preferredDeliveryDate.value || null,
