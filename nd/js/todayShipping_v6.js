$(function(){
    ...
    /************************************************
    ★ 오늘발송 노출 조건 ★ (전용 아이콘 기준 — 기본 OFF)
    - NEW 배지 기준은 폐기: 카페24 '신제품' 자동 진열이 NEW를 자동 부착해
      가꿈이 선택하지 않은 상품(예: 1054)에도 타이머가 켜졌음 (2026-06-10 피드백).
    - 현재 기준: 아이콘 이름(alt)에 "MUST HAVE"(musthave) 또는
      "오늘출발" / "오늘발송" 이 들어간 아이콘이 붙은 상품에만 노출.
      (이미지 파일명에 musthave 가 들어가도 인식)
    - 사용법: 가꿈이 [상품 수정 → 아이콘 설정]에서 전용 아이콘을 만들어
      대상 상품에 부착 + ★아이콘 표시기간을 '제한없음'(또는 유효 기간)으로
      설정 후 저장★ 해야 상품페이지에 출력되고, 출력되면 타이머가 켜짐.
    - 그런 아이콘이 없는 상품(현재 전 상품)은 타이머 숨김(기본 OFF).
    - 만약 만든 아이콘이 인식되지 않으면(이름이 위 규칙과 다름)
