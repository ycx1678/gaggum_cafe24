(function(){
    ...
    function getProductCsvPath(){
        var productNo = getProductNo();
        return productNo ? "/nd/images/simulator/products/" + productNo + "/simulator.csv" : "";
    }

    function normalizeCsvImagePath(path){
        var image = trimText(path);
        var productNo = getProductNo();
        var basePath = productNo ? "/nd/images/simulator/products/" + productNo + "/" : "/nd/images/simulator/products/";
        if(!image) return "";
    ...
    function parseProductCsvConfig(text){
        ...
        rows.slice(1).forEach(function(row, rowIndex){
            ...
            var image = normalizeCsvImagePath(row[fixed.image]);
    ...
    function resolveConfigAsync(callback){
        var csvPath = getProductCsvPath();
        if(!csvPath || typeof window.fetch !== "function"){
            callback(resolveConfig());
            return;
        }

        fetchTextCandidates(getFetchCandidates(csvPath), function(text){
            var csvConfig = text ? parseProductCsvConfig(text) : null;
