$(function(){
    ...
    - 현재 기준: 아이콘 이름(alt) 또는 이미지 파일명에 "MUST HAVE"
      (musthave / must-have / must_have 포함)가 들어간 아이콘이 붙은 상품에만 노출.
    - 사용법: 가꿈이 [상품 수정 → 아이콘 설정]에서 MUST HAVE 아이콘을 만들어
      대상 상품에 부착하면 켜짐. (아이콘 이름 또는 업로드 파일명에
      musthave 가 들어가게 만들어 주세요 — 예: musthave.gif)
    - 그런 아이콘이 없는 상품(현재 전 상품)은 타이머 숨김(기본 OFF).
    - 만약 만든 아이콘이 인식되지 않으면(이름·파일명에 musthave 없음)
      아래 ndTodayShipIconSrc 에 해당 아이콘 파일명을 직접 기입하면 됩니다.
    ************************************************/
    var ndTodayShipIconSrc = ""; // (선택) 예: "icon_202606xxxxxxxxxx.gif" — 직접 지정 시 이 파일명도 인식
    var ndHasTodayShipIcon = $(".icon img, img.icon_img").toArray().some(function(img){
        ...
        return alt.indexOf("musthave") > -1 || /must[-_]?have/.test(src);
    });
