(function () {
    ...
    function fetchPriceInfo(href) {
        ...
        priceCache[url.href] = window.fetch(url.href, { credentials: "same-origin" })
    ...
    function enrichCardPrices(root) {
        var cards = root.querySelectorAll(".nd-cart-recommend__item[data-nd-product-href]");
        Array.prototype.forEach.call(cards, function (item) {
            var price = item.querySelector(".nd-cart-recommend__price");
            fetchPriceInfo(item.getAttribute("data-nd-product-href")).then(function (info) {
                if (!info || !price || !item.parentNode) return;
                renderPriceGroup(price, info.original, info.current);
            });
        });
    }
            ...
            '<section class="nd-cart-recommend__panel" role="dialog" aria-modal="true" aria-labelledby="ndCartRecommendTitle">',
                ...
                '<header class="nd-cart-recommend__header">',
                    '<div class="nd-cart-recommend__heading">',
                        '<h2 class="nd-cart-recommend__title" id="ndCartRecommendTitle">장바구니에 담았어요</h2>',
