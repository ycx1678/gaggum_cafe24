$(function(){
   ...
   ========================= */
    ...
    function getActualBulkBenefitText($row){
        ...
        var $benefit = $td.find("[data-benefit-name], .benefitName, .benefit_name, .ec-benefit, [class*='benefit']").filter(function(){
            return $.trim($(this).attr("data-benefit-name") || $(this).text() || "").length > 0;
        }).first();
        if($benefit.length > 0){
            return $.trim($benefit.attr("data-benefit-name") || $benefit.text() || "").replace(/\s+/g, " ");
        }

        var benefitNameMatch = text.match(/^혜택명\s*[:：]\s*(.+)$/);
        if(benefitNameMatch && $.trim(benefitNameMatch[1])){
            return $.trim(benefitNameMatch[1]);
        }
    ...
    $(".prd_info tr").each(function(){   
        ...
        if(tit_text == "대량구매혜택"){
            var actualBulkBenefitText = getActualBulkBenefitText($(this));
            if(!actualBulkBenefitText){
                $(this).hide().attr("data-nd-hidden-reason", "no-bulk-benefit");
            } else {
                $(this).children("td").text(actualBulkBenefitText);
                $(this).show().attr("data-nd-benefit-visible", "true");
            }
            return;
        }
