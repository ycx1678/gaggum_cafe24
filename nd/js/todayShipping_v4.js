$(function(){
    ...
    /************************************************
    ★ 오늘발송 노출 조건 ★ (NEW 배지 기준 — 가꿈 자가관리)
    - 카페24 자사몰엔 '오늘출발' 기본 기능이 없어, 'NEW' 배지로 제어합니다.
    - 가꿈이 [상품 수정 → 아이콘 설정]에서 'NEW' 배지를 붙인 상품에만 타이머가 노출됩니다.
    - 기본(NEW 미부착)은 타이머를 숨기고 종료합니다.
      (상품 추가/제외는 NEW 배지만 붙였다 떼면 됨 — 코드 수정 불필요)
    ************************************************/
    var ndHasNewBadge = $(".icon img, img.icon_img").toArray().some(function(img){
        return (img.getAttribute("alt") || "").trim().toLowerCase() === "new";
    });
    if (!ndHasNewBadge) {
