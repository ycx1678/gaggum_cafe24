(function () {
  ...
  function currentMemberId() {
    var node = document.getElementById("ndQuoteListMemberId");
    var value = node ? String(node.textContent || "").replace(/\s+/g, "") : "";
    if (!value || value.indexOf("{$") >= 0 || !/^[A-Za-z0-9._@-]+$/.test(value)) return null;
    return value;
  }
    ...
    items.forEach(function (item) {
      ...
      card.appendChild(
        text(
          "p",
          "ndQuoteCardMeta",
          item.channel === "cafe24" ? "장바구니 요청" : "관리자 작성",
        ),
      );
  ...
  ready(function () {
    ...
    fetch(BACKEND + "/api/quote-requests/list", {
      ...
      body: JSON.stringify({ tokens: saved, memberId: currentMemberId() }),
    })
