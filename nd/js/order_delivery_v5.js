(function(){
    ...
    function enhanceOrderForm(){
        ...
        function applyOrderShipping(){
            ...
            var hideParcel = (storedKind !== "parcel");
            var target = null;
            ...
            // ② 택배(선불) 라디오 숨김/복원 (상세가 택배가 아니면 숨김)
            for(var j = 0; j < items.length; j += 1){
                if(items[j].kind === "parcel" && items[j].wrap && items[j].wrap.style){
                    items[j].wrap.style.display = hideParcel ? "none" : "";
                }
            }
