$(function(){
    ...
    /************************************************
    ★ 오늘발송 노출 조건 ★ (상품 아이콘 기반 — 가꿈 자가관리)
    - 카페24 자사몰엔 '오늘출발' 기본 기능이 없어, 상품 '아이콘'으로 제어합니다.
    - 가꿈이 [아이콘 관리]에서 이름(alt)에 "오늘발송"(또는 오늘출발/당일발송)이
      들어간 아이콘을 만들어 대상 상품에 부착하면, 그 상품에만 타이머가 노출됩니다.
    - 미부착 상품이면 타이머를 숨기고 종료합니다.
      (상품 추가/제외는 아이콘만 붙였다 떼면 됨 — 코드 수정 불필요)
    ************************************************/
    var ndTodayShipKeyword = /오늘\s*발송|오늘\s*출발|당일\s*발송/;
    var ndHasTodayShipIcon = $(".icon img, img.icon_img").toArray().some(function(img){
        return ndTodayShipKeyword.test(img.getAttribute("alt") || "");
    });
