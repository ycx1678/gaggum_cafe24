(function () {
  ...
  function productVariantSnapshot(productNo, additional) {
    ...
    if (additional) {
      var additions = window.add_option_data;
      if (typeof additions === "string") {
        try { additions = JSON.parse(additions); } catch { return null; }
      }
      var data = additions && Object.prototype.hasOwnProperty.call(additions, productNo)
        ? additions[productNo] : null;
      if (!data || cleanText(data.option_type).toUpperCase() !== "T") return null;
      mapper = data.option_value_mapper;
    }
  ...
  function parsePublicJson(html, pattern) {
    var mapperMatch = html.match(pattern);
  ...
  function parsePublicVariantMapper(html) {
    return parsePublicJson(html, /\boption_value_mapper\s*=\s*(['"])((?:\\.|[\s\S])*?)\1\s*;/);
  }

  function publicProductHtml(productNo, pages) {
    if (Object.prototype.hasOwnProperty.call(pages, productNo)) return pages[productNo];
    var skin = skinPrefix();
    ...
    pages[productNo] = originalFetch.call(window, productUrl, {
      method: "GET",
    ...
    return pages[productNo];
  }

  // 장바구니는 사람이 읽는 옵션 문구만 남기므로, 이전 장바구니에도 공개 상품
  // 페이지의 실제 Cafe24 메타데이터로 스냅샷을 복원한다. 하나라도 일치하지
  // 않으면 비워 둬 주문 대상을 추측하지 않는다.
  function publicProductSnapshot(productNo, productOptions, pages) {
    return publicProductHtml(productNo, pages).then(function (html) {
      if (!html) return null;
      ...
      // A slash may belong to one option value (e.g. tabletop / leg color).
      // Compare the complete displayed tuple and accept only one actual mapper entry.
      var selectionKey = selectedTexts.map(optionTextKey).join("/");
      var variantMatches = [];
      Object.keys(mapper).forEach(function (key) {
        var values = key.split("#$%");
        if (values.length !== selects.length) return;
        var displayKey = values.join("/").split("/").map(optionTextKey).join("/");
        if (displayKey !== selectionKey) return;
  ...
  function publicAdditionalSnapshot(html, item) {
    var additions = parsePublicJson(html, /\badd_option_data\s*=\s*(['"])((?:\\.|[\s\S])*?)\1\s*;/);
    var productNo = parsePositiveInt(item.productNo, 0);
    if (!additions || !Object.prototype.hasOwnProperty.call(additions, productNo)) return null;
    var data = additions[productNo];
    if (!data || cleanText(data.option_type).toUpperCase() !== "T") return null;
    var productCode = cleanText(data.product_code).toUpperCase();
    if (!PRODUCT_CODE_RE.test(productCode)) return null;
    var mapper = data.option_value_mapper;
    if (typeof mapper === "string") {
      try { mapper = JSON.parse(mapper); } catch { return null; }
    }
    if (!mapper || typeof mapper !== "object" || Array.isArray(mapper)) return null;
    var selectionKey = selectedOptionTexts(item.productOptions).map(optionTextKey).join("/");
    var matches = [];
    Object.keys(mapper).forEach(function (key) {
      var displayKey = key.split("#$%").join("/").split("/").map(optionTextKey).join("/");
      var variantCode = cleanText(mapper[key]).toUpperCase();
      if (selectionKey && displayKey === selectionKey && VARIANT_CODE_RE.test(variantCode) &&
          variantCode.slice(0, 8) === productCode) matches.push(variantCode);
    });
    return matches.length === 1 ? { variantCode: matches[0], cafe24OptionValues: [] } : null;
  }

  function resolveAdditionalSnapshot(item, itemIndex, items, pages) {
    var parentIndex = item.parentSortOrder;
    return Promise.all(items.map(function (parent, index) {
      if (!parent || index === itemIndex || parent.isAdditional || !parent.productNo ||
          (parentIndex != null && parentIndex !== index)) return null;
      return publicProductHtml(parsePositiveInt(parent.productNo, 0), pages).then(function (html) {
        var snapshot = html ? publicAdditionalSnapshot(html, item) : null;
        return snapshot ? { snapshot: snapshot, parentSortOrder: index } : null;
      });
    })).then(function (candidates) {
      var matches = candidates.filter(Boolean);
      // 동일 추가품이 여러 본상품에 걸리면 소속이나 수량을 임의로 배정하지 않는다.
      if (matches.length !== 1) return item;
      var next = applyCapturedSnapshot(item, matches[0].snapshot);
      next.isAdditional = true;
      next.parentSortOrder = matches[0].parentSortOrder;
      return next;
    });
  }
  ...
  function resolveMissingSnapshots(items) {
    ...
    return Promise.all(items.map(function (item) {
      ...
      return publicProductSnapshot(parsePositiveInt(item.productNo, 0), item.productOptions, pages)
        .then(function (snapshot) {
          return snapshot ? applyCapturedSnapshot(item, snapshot) : item;
        });
    })).then(function (resolved) {
      return Promise.all(resolved.map(function (item, index) {
        if (!item || VARIANT_CODE_RE.test(cleanText(item.variantCode).toUpperCase())) return item;
        return resolveAdditionalSnapshot(item, index, resolved, pages);
      }));
    });
  }
