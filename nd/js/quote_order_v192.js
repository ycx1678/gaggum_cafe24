(function () {
    ...
    // v191 is the canonical test-skin asset; the v192 query revision links completed snapshot orders.
    ...
    function enforceOrderForm(context) {
        var hasDiscountCode = Boolean(String(context && context.discountCode || "").trim());
        var notice = banner(
            hasDiscountCode
                ? "견적 전용 할인코드를 Cafe24 주문서에 적용하고 있습니다."
                : "견적 주문: 무통장 입금만 가능합니다. 최종 결제금액을 확인하고 있습니다.",
            false
        );
        var state = { bank: false, total: false, amount: 0, pricingReady: !hasDiscountCode, pricingError: false };
        ...
        function verify() {
            ...
            if (!state.pricingReady) {
                setNotice("견적 전용 할인코드를 Cafe24 주문서에 적용하고 있습니다.", false);
                return;
            }
        ...
        if (hasDiscountCode) {
            applyQuoteDiscountCode(context).then(function () {
                ...
                state.total = false;
                actionableBanner(
                    error.message || "견적 할인코드 적용에 실패했습니다.",
                    "할인코드 다시 적용",
