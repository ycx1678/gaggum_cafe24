    ...
    function initOptionButtons(){
        ...
        $(".xans-product-detail table.xans-product-option select").each(function(){
            ...
            function scrollNextOption($current){
                ...
                if($next.length){
                    var $optionScroller = $current.closest(".info_inner");
                    if($optionScroller.length){
                        var scroller = $optionScroller[0];
                        var scrollerRect = scroller.getBoundingClientRect();
                        var nextRect = $next[0].getBoundingClientRect();
                        scroller.scrollTo({
                            top: Math.max(0, scroller.scrollTop + nextRect.top - scrollerRect.top - 18),
                            behavior: "smooth"
                        });
                    } else {
