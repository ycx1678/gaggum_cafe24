(function(){
    ...
    var ASSET_VERSION = "20260720v198";
    var mobileScrollY = 0;
    var viewportInsetRaf = 0;
    ...
    function fetchJson(url, callback){
        ...
        window.fetch(url, { cache: "no-store", credentials: "omit" }).then(function(response){
            if(!response || !response.ok){
                callback(null, false, response ? response.status : 0);
                return null;
            }
            return response.json();
        }).then(function(json){
            if(json === null || json === undefined) return;
            callback(json, true, 200);
        }).catch(function(){
            callback(null, false, 0);
        });
    }
    ...
    function getMobileViewportBottomInset(){
        if(((window.innerWidth || document.documentElement.clientWidth || 0) > 1024)) return 0;
        var layoutHeight = window.innerHeight || document.documentElement.clientHeight || 0;
        var viewport = window.visualViewport;
        var inset = 0;
        if(viewport && layoutHeight){
            inset = layoutHeight - (viewport.offsetTop + viewport.height);
        }
        if(!isFinite(inset) || inset < 0) inset = 0;
        if(inset < 1 && /iPhone|iPod/i.test(window.navigator.userAgent || "")){
            inset = 100;
        }
        return Math.min(120, Math.max(0, Math.ceil(inset)));
    }

    function updateMobileViewportInset(){
        if(!document.documentElement) return;
        var inset = getMobileViewportBottomInset();
        document.documentElement.style.setProperty("--nd-mobile-browser-inset", inset + "px");
    }

    function scheduleMobileViewportInsetUpdate(){
        if(viewportInsetRaf) return;
        viewportInsetRaf = window.requestAnimationFrame ? window.requestAnimationFrame(function(){
            viewportInsetRaf = 0;
    ...
    onReady(function(){
        ...
        if(root.getAttribute("data-nd-simulator-mounted") === "true") return;
        root.setAttribute("data-nd-simulator-mounted", "true");
        ...
        function createDock(){
            ...
            dock.innerHTML = [
                ...
                '<p class="ndSimulatorColorNotice">※ 모니터 해상도, 밝기, 조명에 따라 실제 상품과 색상 차이가 있을 수 있습니다.</p>',
        ...
        function restoreMobileFixedActionBar(){
            if(!isMobile()) return;
            var orderFixArea = document.getElementById("orderFixArea");
            if(!orderFixArea) return;
            orderFixArea.hidden = false;
            orderFixArea.removeAttribute("aria-hidden");
            if(orderFixArea.style){
                orderFixArea.style.removeProperty("display");
                orderFixArea.style.removeProperty("visibility");
                orderFixArea.style.removeProperty("opacity");
                orderFixArea.style.removeProperty("pointer-events");
                orderFixArea.style.removeProperty("transform");
            }
        }
        ...
        function absoluteSnapshotUrl(src){
            var candidates = getAssetCandidates(src);
            var candidate = candidates.length ? candidates[0] : src;
            if(!candidate) return "";
            try {
        ...
        function getSimulatorSnapshot(){
            syncFromNative();
            var sources = [];
            if(config.baseImage) sources.push(config.baseImage);
            sources = sources.concat(getResolvedLayerImages());
            var layers = sources.map(absoluteSnapshotUrl).filter(Boolean);
            if(!layers.length) return null;
            return {
                productNo: getProductNo(),
                productOptions: getSelectionText(),
                selectedValues: groups.map(function(group){
                    var option = findOptionByValue(group, state[group.key]);
        ...
        function saveSimulatorSnapshot(){
            var snapshot = getSimulatorSnapshot();
            if(!snapshot || !snapshot.productNo) return;
            try {
                var key = "gaggum_quote_simulator_snapshots";
                var saved = JSON.parse(window.localStorage.getItem(key) || "[]");
                if(!Array.isArray(saved)) saved = [];
                saved = saved.filter(function(item){
                    return !item || String(item.productNo) !== String(snapshot.productNo) ||
                        String(item.productOptions) !== String(snapshot.productOptions);
                });
                saved.unshift(snapshot);
                window.localStorage.setItem(key, JSON.stringify(saved.slice(0, 30)));
            } catch(error) {
                /* Safari private mode/storage limits must not block Cafe24 cart actions. */
            }
        }

        window.ND_OPTION_SIMULATOR_GET_SNAPSHOT = getSimulatorSnapshot;
        document.addEventListener("click", function(event){
            var target = event.target;
            if(target && target.closest && target.closest(".xans-product-detail .actionCart")){
        ...
        function hidePreviewFromExternalControl(){
            hidePreview();
            if(document.body){
                document.body.classList.remove("nd-simulator-preview-open");
            }
        ...
        window.ND_OPTION_SIMULATOR_HIDE_PREVIEW = hidePreviewFromExternalControl;
        document.addEventListener("nd:simulator:hidePreview", hidePreviewFromExternalControl);
        ...
        function bindMobileViewportListeners(){
            if(window.__ndSimulatorMobileViewportBound) return;
            window.__ndSimulatorMobileViewportBound = true;
            ...
            window.addEventListener("scroll", scheduleMobileViewportInsetUpdate, { passive: true });
            window.addEventListener("resize", scheduleMobileViewportInsetUpdate);
            window.addEventListener("orientationchange", scheduleMobileViewportInsetUpdate);
            window.addEventListener("pageshow", scheduleMobileViewportInsetUpdate);
            if(window.visualViewport){
                window.visualViewport.addEventListener("resize", scheduleMobileViewportInsetUpdate);
                window.visualViewport.addEventListener("scroll", scheduleMobileViewportInsetUpdate);
            }
            updateMobileViewportInset();
        }
