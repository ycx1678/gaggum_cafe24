(function () {
  ...
  // 스냅샷으로 보관한다.
  function productVariantSnapshot(productNo) {
    ...
    if (optionType === "E") {
      var productCode = cleanText(window.sProductCode).toUpperCase();
      var linkedVariantCode = productCode + "000A";
      if (!PRODUCT_CODE_RE.test(productCode) || !VARIANT_CODE_RE.test(linkedVariantCode)) {
        return null;
      }
      return {
        variantCode: linkedVariantCode,
        cafe24OptionValues: linkedOptionValues,
      };
    }
  ...
  function publicLinkedSnapshot(productNo, productOptions) {
    var selectedTexts = selectedOptionTexts(productOptions);
    if (!productNo || !selectedTexts.length) return Promise.resolve(null);

    var skin = (window.location.pathname.match(/^\/skin-skin\d+/) || [""])[0];
    ...
    return originalFetch.call(window, productUrl, {
      ...
      var variantCode = productCode + "000A";
      if (!PRODUCT_CODE_RE.test(productCode) || !VARIANT_CODE_RE.test(variantCode)) return null;
      ...
      if (
        selects.length !== selectedTexts.length ||
        !selects.length ||
        cleanText(selects[0].getAttribute("option_type")).toUpperCase() !== "E"
      ) {
        return null;
      }
      ...
      return { variantCode: variantCode, cafe24OptionValues: optionValues };
    }).catch(function () {
      return null;
    });
  }
  ...
  function resolveMissingSnapshots(items) {
    ...
    return Promise.all(items.map(function (item) {
      ...
      return publicLinkedSnapshot(parsePositiveInt(item.productNo, 0), item.productOptions)
  ...
  function readSelectedRow(row, additional) {
    ...
    var snapshot = additional ? null : productVariantSnapshot(productNo);
    return {
      productNo: productNo,
