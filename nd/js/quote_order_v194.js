(function () {
    ...
    var QUOTE_SERVICE_PRODUCT_NO = 1090;
    ...
    function removeOrphanQuoteServiceItems() {
        ...
        apiCall("getCartList", []).then(function (response) {
            var serviceItems = (response && response.carts || []).filter(function (item) {
                return Number(item && item.product_no || 0) === QUOTE_SERVICE_PRODUCT_NO;
            }).map(function (item) {
                return {
