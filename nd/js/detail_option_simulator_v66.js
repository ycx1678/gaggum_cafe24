(function(){
    ...
    function getAssetCandidates(src){
        ...
        if(prefix && src.indexOf("/nd/") === 0){
            return [prefix + src, src].map(addAssetVersion);
        }
        if(!prefix && src.indexOf("/nd/") === 0){
    ...
    onReady(function(){
        ...
        if(!config || !root || !infoInner) return;
        ...
        function createDock(){
            ...
            if(optionTable && optionTable.parentNode === infoInner){
